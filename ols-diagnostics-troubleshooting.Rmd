...
A similar robust variance estimator commonly used in political science are panel corrected standard errors (PCSE) of Beck Katz.

In a TSCS 
$$
y_{i,t} = \vec{x}_{i,t}' \beta + \varepsilon_{i,t}
$$
for $i = 1, \dots, N$, and $t = 1, \dots, T$.

1. Errors are constant within units, $\varepsilon_{i,t} = \varepsilon_{i,s}$ for all $t, s$, but not across units.
2. Errors are time-invariant
3. Errors are temporally uncorrelated
4. Errors are correlated across units contemporaneously.

The error structure is represented as,
$$
\mat\Sigma  = 
\left\[
\begin{array}{llll}
\tilde{\mat{Sigma}} & \mat{0} & \cdots & \mat{0} \\
\mat{0} \tilde{\mat{Sigma}}  & \cdots & \mat{0} \\
\vdots & \vdots & \ddots & vdots \\
\mat{0} & \mat{0} & \cdots & \tilde{\mat{Sigma}} 
\end{array}
\right\]
$$
where $\mat{\Sigma}$ is an $N T \times N T$ matrix, 
and $\tilde{\mat{Sigma}}$ is an $N \times N$ matrix with the contemporaneous correlation between units,
$$
\tilde{\mat\Sigma} = 
\begin{bmatrix} 
...
Some notes on using PCSE 

- It relies on large $T$ asymptotics. Thus, it works better as the number of time periods increases.
- Since it assumes that there is no correlation across time, that needs to be handled in other ways, such as including a lagged dependent variable.
...
1. If the sample size is small, normal errors are required for correct confidence interval coverage and p-values in tests. In large samples, CLT properites of OLS kick in and the normality of errors assumption is not needed to justify the sampling distributions of the test statistics.
2. Heavy-tailed errors threaten the efficiency of OLS estimate.
3. Skewed or multimodal errors suggest that the conditional mean $E(Y | \mat{X})$, estimated by OLS may not be a good summay of the data.
...
1. Transform the dependendt variable and use OLS
2. Add different sets of covariates. This is especially likely with multimodal error distributions, which could suggest an omitted categorical variable.
2. Use a different model other than OLS.


**R** Calculate studentized residuals with the function `rstudent` and a QQ-plot using [stat_qq](http://docs.ggplot2.org/current/stat_qq.html) in ggplot.
