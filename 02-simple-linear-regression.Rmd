...
In more complicated problems, the soluations have to be found through an iterative optimization problem.
...
Those observations far from the average value of $x$ are given more weight.[^outlier]

See the paper by XXXX on panel data and regression weighting.

[^outlier]: Though whether those observations, if dropped, would affect the values of $\hat{\beta}_i$ depends on whether the outcome of that observation is similar to its predicted value.
