...
The non-parametric bootsrap resamples the data with replacement $B$ times and calculates the statistic on each resample.
...
-   In small samples there is a narrowness bias. [@Hesterberg, p. 24]. As always, small samples is problematic.
...
Bootstrapping is only used to generate standard errors and confidence intervals.
...
For some Monte Carlo results on the accuracy of the bootstrap see @Hesterberg2015, p. 21.
